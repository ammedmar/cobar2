...
\section{Categorical comparison}\label{s:categorical}

\todo{little intro}

\subsection{Triangulation and cubification}
...
\todo{Is the word ``independently" justified? Just making sure we are not commenting on a divisive issue.}
...
\subsection{Rigidification and the extended cobar construction}

The next result, which is an observation of the second named-author and Zeinalian, relates the cubical cobar construction and the rigidification functor via the triangulation functor as defined in \cref{ss:triangulation and its adjoint}.\todo{Please define the triangulation functor here intuitively providing a reference. A useful piece of code
\[
\triangulate \colon \cSet \leftrightarrows \sSet : \cubify
\]
}
...
\todo{What's the idea of the map? Maybe give the intuition and the reference, and then the proposition will be of the form: The simplicial map $\psi_{\cG}$ is a natural weak equivalence for any simplicial group $\cG$.}

\begin{proposition} \label{p:hinich}
	For any simplicial group $\cG \in \sGrp$, there is a natural weak homotopy equivalence
	\[
	\psi_{\cG} \colon \classifying(\cG) \xrightarrow{\simeq} \nerve\iota(\cG).
	\]
\end{proposition}
...
\begin{lemma} \label{CandG}
	...
	\todo{As mentioned before, I am not a big fan of this format. I find it better to say what the map is before and then state in the lemma its properties. This is something discussed by Serre in his "How to write mathematics badly."}
\end{lemma}
...
\begin{corollary} \label{widehatgcobarandG}
	...
	\]
	for any reduced simplicial set $X$.
	\todo{Maybe explicit about the map instead of existence quantifier? One can put the proof before where the map is mentioned.}
\end{corollary}
...
\begin{equation}\label{e:factorization of HT}
	\HT \colon \adamsE(X) \cong \cchains(\ccobarE(X)) \xra{\EZ} \schains(\triangulate\ccobarE(X)) \to \schains(\kan(X)),
\end{equation}
...
\todo{I would suggest to the "shuffle quasi-isomorphism" as the Eilenberg--Zilber map using $\xra{\EZ}$ above.}